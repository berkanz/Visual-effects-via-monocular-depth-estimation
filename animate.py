...
def cl_parser():
    ...
    parser.add_argument('--gif_duration', default=6, type=float, help='total duration of output gif (in seconds)')
    ...
    parser.add_argument('--visualize_pointcloud', default=False, type=bool, help='flag for visualizing point cloud')
...
def main():
    ...
    fov_x = np.deg2rad(55)
    fov_y = (img_resized.shape[0]/img_resized.shape[1]) * fov_x
    ...
    assert cleaned_pcd.has_colors(), "Colors could not be added to the point cloud"
    if args.visualize_pointcloud:
        open3d.visualization.draw_geometries([cleaned_pcd])
    
    
    mtl = open3d.visualization.rendering.Material()
    mtl.shader = "defaultUnlit"
    
    current_center = np.array([0, 0, 1])  
    current_eye = np.array([0, 0, 0])  
    current_up = np.array([0, -180, 10])  
    current_FOV = np.rad2deg(fov_x)
    
    final_center = np.array([0.0, 0.0, 1.0])  
    final_eye = np.array([0, 0, 0.85]) 
    final_up = np.array([0, -180, 0])  
    final_FOV = 110
